...
from sqlalchemy import Index, Integer, String, Enum, DateTime, Date, ForeignKey, UniqueConstraint
...
class UserLoginLog(Base):
    """
    用户每日登录记录表，用于统计连续登录天数。
    """
    __tablename__ = "user_login_log"

    __table_args__ = (
        UniqueConstraint("user_id", "login_date", name="user_login_date_unique"),
        Index("idx_user_login_log_user_id", "user_id"),
        Index("idx_user_login_log_login_date", "login_date"),
    )

    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True, comment="登录记录ID")
    user_id: Mapped[int] = mapped_column(Integer, ForeignKey(User.id), nullable=False, comment="用户ID")
    login_date: Mapped[date] = mapped_column(Date, default=date.today, nullable=False, comment="登录日期")
    login_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.now, nullable=False, comment="登录时间")

    def __repr__(self):
        return f"<UserLoginLog(id={self.id}, user_id={self.user_id}, login_date={self.login_date})>"
